...
Wraith.prototype.config = function(config) {
	...
	for(var url in self.urls) {
		var name = self.urls[url].name || self.urls[url],
	...
	if( self.engines.length === 1 ) {
		for(var domain in self.domains) {
			for(url in self.urls) {
				...
				name = urlData.name || urlData;
				imageUrls.push(self.domains[domain] + path);
				self.folder = Path.join(self.outputFolder, name.replace(/\/+$/,'') + '/');
		...
		for(url in self.urls) {
			...
			for(var engine in self.engines) {
				...
				folder = Path.join(self.outputFolder, name.substring(1).replace(/\/+$/,'') + '/');
				for(size in self.sizes) {
