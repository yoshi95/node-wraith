...
function getFolder(data) {
	var name = data.name;
	return name ? name : data.substring(1).replace(pathReplace,'')
}
...
Wraith.prototype.config = function(config) {
	...
	self.renderDelay		= config.renderDelay || 0
	...
	for(var url in self.urls) {
		var folder = Path.join(self.outputFolder,
					getFolder(self.urls[url])+ '/');
...
Wraith.prototype.takeScreenshots = function () {
	...
	if( self.engines.length === 1 ) {
		for(var domain in self.domains) {
			for(url in self.urls) {
				urlData = self.urls[url];
				path = urlData.path || urlData;
				imageUrls.push(self.domains[domain] + path);
				self.folder = Path.join(self.outputFolder,
					getFolder(urlData) + '/');
		...
		for(url in self.urls) {
			...
			for(var engine in self.engines) {
				imageUrls.push(self.domains[0] + path);
				folder = Path.join(self.outputFolder,
					getFolder(urlData) + '/');
				for(size in self.sizes) {
					image = folder + self.engines[engine] + '_' + self.sizes[size] + '.png';
					screenshotQueue.push({
						engine: self.engines[engine],
						url: self.domains[0] + path,
						selector: urlData.selector,
						size: self.sizes[size],
						output: image
					});
	...
	async.eachLimit(screenshotQueue, self.maxConnections, function(task, callback) {
		webshot(task.url, task.output, {
			...
			captureSelector: task.selector || false,
...
Wraith.prototype.compareScreenshots = function() {
	...
	for(var url in self.urls) {
		var dir = getFolder(self.urls[url]) + '/';
